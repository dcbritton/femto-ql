...
std::shared_ptr<EvaluationNode> convert(std::shared_ptr<node> boolExprRoot, EntryIterator& entryIteratorReference) {

    // (bool_expr)
    if (boolExprRoot->components.size() == 1 /*only child: boolExprRoot->components[0]->type == bool_expr*/) {
        ParensNode pn;
        pn.subExpr = convert(boolExprRoot->components[0], entryIteratorReference);
    ...
    // !(bool_expr)
        ...
        NotNode nn;
        nn.subExpr = convert(boolExprRoot->components[1], entryIteratorReference);
    ...
    // bool_expr op_or bool_expr
        ...
        OrNode on;
        on.lhs = convert(boolExprRoot->components[0], entryIteratorReference);
        on.rhs = convert(boolExprRoot->components[2], entryIteratorReference);
    ...
    // bool_expr op_and bool_expr
        ...
        AndNode an;
        an.lhs = convert(boolExprRoot->components[0], entryIteratorReference);
        an.rhs = convert(boolExprRoot->components[2], entryIteratorReference);
